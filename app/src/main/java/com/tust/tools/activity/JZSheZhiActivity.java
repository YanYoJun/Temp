...
public class JZSheZhiActivity extends Activity implements OnClickListener {
	...
	private void init() {
		...
		qingchu_rl = (RelativeLayout) this
				.findViewById(R.id.jz_shezhi_qingchu_rl);
		...
		bianjijiami = (LinearLayout) this
				.findViewById(R.id.jz_shezhi_bianjimima_ll);
	...
	public void onClick(View v) {
		...
		case R.id.jz_shezhi_queding_bt:
			if (mi == 0 || tempmi == -1) {// 当没有密码时
				...
				if (mima1.getText().toString().length() != 6
						|| mima2.getText().toString().length() != 6) {
					showMsg("密码必须为六位数");
					return;
				}
				if (!mima1.getText().toString()
						.equals(mima2.getText().toString())) {
					showMsg("两次输入的密码不一致");
					return;
				}
